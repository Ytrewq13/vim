...
let s:TEMPLATE_DIR = '~/.config/vim_templates/'
...
function! FindAndFillTemplate (filepath) abort
    ...
    let allfiles = globpath(expand(s:TEMPLATE_DIR), "*", 0, 1)
    let templatefiles = globpath(expand(s:TEMPLATE_DIR), "template.*", 0, 1)
    ...
    " Extract template logic out of if statements - into function

    " Exact filename, case-sensitive
    if filereadable(expand(s:TEMPLATE_DIR) . filename)
        let template = join( readfile(expand(s:TEMPLATE_DIR) . filename), "\n")
    ...
    elseif !empty(matchstr(allfiles, filename))
        let tfilename = matchstr(allfiles, filename)
        let template = join( readfile(tfilename), "\n")
        ...
        " Install the filled-in template into the empty buffer...
        call append(0, split(template, "\n"))
    " Case-sensitive file extension
    elseif filereadable(expand(s:TEMPLATE_DIR) . 'template.' . extension)
        ...
        " Install the filled-in template into the empty buffer...
        call append(0, split(template, "\n"))
"    " TODO: Case-insensitive file extension
    endif
endfunction
