...
" TODO: add seperate binds for staying on the same line or for moving to the
" next line
...
augroup CommentSupport
    autocmd!
    autocmd FileType             *sh,awk,perl,perl6,ruby           let b:CT_EOL_COMMENT = get(b:, 'CT_EOL_COMMENT', '#')
    autocmd FileType             python                            let b:CT_EOL_COMMENT = '# '
    autocmd FileType             vim                               let b:CT_EOL_COMMENT = get(b:, 'CT_EOL_COMMENT', '"')
    autocmd FileType             tex                               let b:CT_EOL_COMMENT       = '%'
    autocmd FileType             haskell,agda                      let b:CT_EOL_COMMENT       = '-- '
    autocmd FileType             haskell,agda                      let b:CT_DELIMITED_COMMENT = { 'start': '{-', 'end': '-}' }
    autocmd FileType             c                                 let b:CT_EOL_COMMENT       = '//'
    autocmd FileType             c                                 let b:CT_DELIMITED_COMMENT = { 'start': '/*', 'end': '*/' }
